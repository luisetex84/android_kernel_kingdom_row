...
int diag_process_apps_pkt(unsigned char *buf, int len)
{
	...
	/*[BEGIN]leijl1,20130730,add NV browser reset target function*/	
	/* Check for reset modem command */	
	else if((*buf == 0x29) && (*(buf+1) == 0x02)) {	
		driver->apps_rsp_buf[0] = *buf;
		encode_rsp_and_send(0);
		msleep(5000);
		/* NV browser reset target */
		printk(KERN_CRIT "diag:NV browser reset target, Rebooting SoC..\n");
		kernel_restart(NULL);
		/* Not required, represents that command isnt sent to modem */
		return 0;
	}
	/*[END]leijl1,20130730,add NV browser reset target function*/
