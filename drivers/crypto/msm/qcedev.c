...
#include "qce.h"
//yangjq, 20140507, Acquire wakelock to fix 50mA gcc_ce2_clk's active issue
...
//yangjq, 20140507, Acquire wakelock to fix 50mA gcc_ce2_clk's active issue, START
static uint32_t qcedev_count;
static struct wake_lock qcedev_wakelock;
//yangjq, 20140507, Acquire wakelock to fix 50mA gcc_ce2_clk's active issue, END
...
//yangjq, 20140507, Acquire wakelock to fix 50mA gcc_ce2_clk's active issue, START
	if(--qcedev_count == 0)
#if 0
		wake_unlock(&qcedev_wakelock);
#else
		wake_lock_timeout(&qcedev_wakelock, 2* HZ);
#endif
//yangjq, 20140507, Acquire wakelock to fix 50mA gcc_ce2_clk's active issue, END
	return 0;
}
...
//yangjq, 20140507, Acquire wakelock to fix 50mA gcc_ce2_clk's active issue, START
		qcedev_count = 0;
		wake_lock_init(&qcedev_wakelock, WAKE_LOCK_SUSPEND,
			"qcedev_wakelock");
//yangjq, 20140507, Acquire wakelock to fix 50mA gcc_ce2_clk's active issue, END
		return 0;
	}
