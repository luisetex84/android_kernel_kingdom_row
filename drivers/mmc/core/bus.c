...
int mmc_check_life_time(struct mmc_card *card,int mode);

static ssize_t mmc_report_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
    ...
    switch (card->type) {
        case MMC_TYPE_MMC:
            mmc_claim_host(host);
			eof = mmc_check_life_time(card,1);
			lifetime = mmc_check_life_time(card,0);
...
#if 1 //lenovo.sw :
	__ATTR(report, S_IRUGO, mmc_report_show, NULL),
#endif //
	__ATTR_NULL,
};
