...
int mmc_check_life_time(struct mmc_card *card,int mode)
{
	...
	mmc_send_ext_csd(card, l_buf);

	value = l_buf[268];
	printk ("DEVICE_LIFE_TIME_ESTIMATION\n");
	printk ("%d - %d device life time used\n\n", (value - 1) * 10, value * 10);
	...
	case 3 :
		...
		break;
	default :
		printk ("Uncorrectable result occured!!\n");
		break;
	}
	if(mode)
		value = l_buf[267];//eof
	else
		value = l_buf[269];//lifetime
	return value;
}
