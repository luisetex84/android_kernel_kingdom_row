...
static int otg_hack_active = 0; /* default should be 0 */

module_param_named(otg_hack_active, otg_hack_active, int, 0664);
    ...
    if (otg_hack_active) {
    	if (host_mode) {
            if (!test_bit(B_SESS_VLD, &dotg->inputs)) {
                ...
                dev_err(otg->phy->dev, "force OTG charging\n");
                charger->chg_type = DWC3_SDP_CHARGER;
                dwc3_otg_set_power(dotg->otg.phy, 500);
            }
        }
	    else {
            if (charger->chg_type != DWC3_INVALID_CHARGER)
...
static void dwc3_otg_sm_work(struct work_struct *w)
{
	...
	case OTG_STATE_UNDEFINED:
		...
		if (!dotg->psy) {
			...
			dotg->psy = power_supply_get_by_name("usb_otg");
		...
		if (test_bit(ID, &dotg->inputs)) {
            ...
            if (otg_hack_active) {
                if ((jiffies <= (last_jiffies + HZ))
                    ...
                    dev_dbg(phy->dev, "give some time for charger detection\n");
                    work = 1;
                    delay = last_jiffies + HZ - jiffies;
                    break;
                }
            }
