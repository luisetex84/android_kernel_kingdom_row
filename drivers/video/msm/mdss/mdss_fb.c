...
struct timeval begin_time, end_time, commit_time;
...
void mdss_fb_update_backlight(struct msm_fb_data_type *mfd)
{
	...
	if (mfd->unset_bl_level && !mfd->bl_updated) {
		...
		if ((pdata) && (pdata->set_backlight)) {
			...
			time_escape = ((end_time.tv_sec - commit_time.tv_sec) * 1000000 + end_time.tv_usec - commit_time.tv_usec)/ 1000;
			printk("XXXXXXXXXXXXXXX %s:from commit_pending to bl open time %dms XXXXXXXXXXXXXXX\n", __func__, time_escape);
			if (time_escape > 1000) {
				printk("################# commit pending too long %d #######################\n", time_escape);
			}
			time_escape = ((end_time.tv_sec - begin_time.tv_sec) * 1000000 + end_time.tv_usec - begin_time.tv_usec)/ 1000;
			printk("XXXXXXXXXXXXXXX %s:from set bl to bl open time %dms XXXXXXXXXXXXXXXXX\n", __func__, time_escape);
			if (time_escape > 1000) {
				printk("################# bl open too long %d #######################\n", time_escape);
			}
		}
	}
}
...
/*zhujp2 kpdbl enable end*/
	...
	default:
		if (mfd->panel_power_on && mfd->mdp.off_fnc) {
			...
			del_timer_sync(&mfd->no_update.timer);
...
static int mdss_fb_blank(int blank_mode, struct fb_info *info)
{
	...
	printk("======= %s %ldms =======\n", __func__, ((end.tv_sec - begin.tv_sec) * 1000000 + end.tv_usec - begin.tv_usec)/ 1000);
...
static int __mdss_fb_display_thread(void *data)
{
	...
	while (1) {
		...
		do_gettimeofday(&commit_time);
		ret = __mdss_fb_perform_commit(mfd);
		atomic_dec(&mfd->commits_pending);
		wake_up_all(&mfd->idle_wait_q);
		atomic_inc(&commits_qos_param);
		wake_up_all(&display_qos_load);
	}
